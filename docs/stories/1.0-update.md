...
The new [Stories](https://github.com/angular/angular-cli/wiki/stories) section covers common usage scenarios, so be sure to have a look!

Below are the changes between a project generated two months ago, with `1.0.0-beta.24` and a `1.0.0` project.
...
现在，您可以在`defaults`中列出每个生成器([#4389](https://github.com/angular/angular-cli/pull/4389))的生成器默认值。
...
您可以列出标志，因为它们出现在[生成器命令](https://github.com/angular/angular-cli/wiki/generate-component)中:
